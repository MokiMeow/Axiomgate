...
- [-] E4 CLI (`doctor`, `mission create/run/resume/verify/receipt`, `receipt verify`, `replay`) + npm distribution. The bundled `axiomgate@0.1.0` tarball, fresh install, doctor/help, three production-logic replay scenarios, offline receipt tamper rejection, and MCP stdio path are verified; `npm publish` is a user action. Evidence: [`evidence/public/publish-prep-verification.md`](../evidence/public/publish-prep-verification.md), `packages/axiomgate-core/test/replay.test.ts`.
